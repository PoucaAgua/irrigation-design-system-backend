...
class CropCoefficientRepository:
    ...
    def create_crop_coefficient(self, crop_name: str, crop_type: str, kc_initial: float, kc_mid_season: float, kc_final: float) -> models.CropCoefficientModel:
        db_crop_coefficient = models.CropCoefficientModel(crop_name=crop_name, crop_type=crop_type, kc_initial=kc_initial, kc_mid_season=kc_mid_season, kc_final=kc_final)
        self.session.add(db_crop_coefficient)
        self.session.commit()
        self.session.refresh(db_crop_coefficient)
        return db_crop_coefficient

    def get_crop_coefficient(self, crop_coefficient_id: int) -> Optional[models.CropCoefficientModel]:
        return self.session.query(models.CropCoefficientModel).filter(models.CropCoefficientModel.id == crop_coefficient_id).first()
    ...
    def update_crop_coefficient(self, crop_coefficient_id: int, crop_name: str, crop_type: str, kc_initial: float, kc_mid_season: float, kc_final: float) -> Optional[models.CropCoefficientModel]:
