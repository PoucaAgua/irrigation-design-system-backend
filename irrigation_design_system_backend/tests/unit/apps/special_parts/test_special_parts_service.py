...
class TestConsultNominalDiameterTable:
    ...
    @pytest.mark.parametrize("diameter, types, expected", [
        (Decimal('1.00'), [SpecialPartInput(1, SpecialPartsTypes.Degree_90_Elbow)], Decimal('1.5')),
        (Decimal('2.00'), [SpecialPartInput(2, SpecialPartsTypes.Open_Gate_Valve)], Decimal('1.6')),
        (Decimal('3.00'), [SpecialPartInput(10, SpecialPartsTypes.Degree_90_Lateral_Tee)], Decimal('80')),
    ])
    ...
    def test_special_parts_load_loss(self, diameter, types, expected):
        ...
        assert (
            Decimal(SpecialPartsService.special_parts_load_loss(special_parts_input)) - Decimal(expected) < self.error
        )
